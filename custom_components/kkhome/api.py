...
class KKHomeApiClient:
    ...
    async def _request(
        ...
        *,
        ...
        if response.status_code in (401, 403) or (
            ...
            if not allow_unauthenticated and retry_on_auth_failure and self._credentials_available():
                ...
                return await self._request(
                    ...
                    allow_unauthenticated=False, retry_on_auth_failure=False, unwrap_data=unwrap_data,
                )
            raise KKHomeAuthError((parsed.get("msg") if isinstance(parsed, dict) else None) or "Not logged in")
        ...
        if isinstance(parsed, dict):
            ...
            if unwrap_data and "data" in parsed:
                return parsed["data"]
        return parsed
    ...
    def _find_token(self, payload: Any) -> str | None:
        token_keys = {
            "accesstoken", "access_token", "token", "bearertoken",
            "authorization", "jwt", "jwttoken", "logintoken", "usertoken",
            "sessiontoken", "sessionid", "sid",
        }
        if isinstance(payload, str) and len(payload) >= 12:
            return payload
        if isinstance(payload, dict):
            for key, value in payload.items():
                if str(key).lower() in token_keys and isinstance(value, str) and len(value) >= 12:
                    return value
